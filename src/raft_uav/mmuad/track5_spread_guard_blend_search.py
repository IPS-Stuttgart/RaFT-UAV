"""Train-fold search for Track 5 spread-guard fallback blending.

The spread-guard ensemble can fall back to a trusted branch when ensemble
trajectories disagree.  A hard fallback is sometimes too conservative because the
weighted mean can still carry useful corrective signal.  This helper extends the
spread-guard search by selecting the fallback blend factor on a labeled train
fold, then writing a reusable leaderboard-safe config.
"""
...
from raft_uav.mmuad.track5_estimate_ensemble_spread_guard import _read_estimate_csv
from raft_uav.mmuad.track5_estimate_ensemble_spread_guard import build_spread_guarded_estimate_ensemble
from raft_uav.mmuad.track5_estimate_ensemble_spread_guard import write_spread_guard_outputs

SPREAD_GUARD_BLEND_GRID_CSV = "mmuad_track5_spread_guard_blend_search_grid.csv"
SPREAD_GUARD_BLEND_BEST_JSON = "mmuad_track5_spread_guard_blend_best_config.json"
BEST_OUTPUT_DIR = "best_spread_guard_blend_submission"
...
def search_track5_spread_guard_blend_settings(
    ...
    *,
    ...
    """Evaluate threshold/policy/blend rows and return grid plus best config."""
    ...
    blends = tuple(_validate_blend(value) for value in fallback_blends)
    if not thresholds:
        raise ValueError("at least one spread threshold is required")
    if not blends:
        raise ValueError("at least one fallback blend is required")
                    ...
                    records.append(
                        {
                            ...
                            "fallback_blend": float(blend),
                            "guard_applied_rows": int(
                                estimates["spread_guard_applied"].astype(bool).sum()
                            )
    ...
    best = {
        "schema": "raft-uav-mmuad-track5-spread-guard-blend-search-v1",
        ...
        "fallback_blend": _jsonable(best_row["fallback_blend"]),
        "max_nearest_time_delta_s": max_nearest_time_delta_s,
        "metrics": {
...
def write_spread_guard_blend_search_outputs(
    *,
    estimate_inputs: Iterable[EstimateInput],
    template: pd.DataFrame,
    truth: pd.DataFrame,
    ...
    if write_best_submission:
        ...
        best_paths = write_spread_guard_outputs(
            ...
            fallback_label=fallback_label,
            fallback_blend=float(best["fallback_blend"]),
            max_nearest_time_delta_s=max_nearest_time_delta_s,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="raft-uav-mmuad-track5-spread-guard-blend-search",
        description="select Track 5 spread-guard fallback blending on a labeled split",
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="fallback blend in [0,1]; may be repeated or comma-separated",
    )
    parser.add_argument(
        "--fallback-policy",
        action="append",
    ...
    blends = _parse_float_list(args.fallback_blend) or (0.0, 0.25, 0.5)
    ...
    print("mmuad_track5_spread_guard_blend_search=ok")
...
def _validate_blend(value: float) -> float:
    ...
    if not np.isfinite(blend) or not 0.0 <= blend <= 1.0:
        raise ValueError(f"fallback blend must be finite and in [0,1]: {value}")
    return blend
