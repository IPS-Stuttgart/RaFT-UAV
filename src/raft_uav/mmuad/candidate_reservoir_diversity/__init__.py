"""Compatibility package for branch-column-aware diversity summaries.

The maintained implementation lives in the sibling
``candidate_reservoir_diversity.py`` module. This package preserves the public
import path while ensuring summary diagnostics use the same branch column as
...
if _SPEC is None or _SPEC.loader is None:  # pragma: no cover
    raise ImportError(f"cannot load candidate reservoir diversity from {_IMPL_PATH}")
...
def diversity_cap_summary(
    ...
    *,
    ...
    return {
        ...
        "input_candidates_per_frame_mean": _IMPL._mean(_IMPL._frame_counts(input_rows)),
        "output_candidates_per_frame_mean": _IMPL._mean(_IMPL._frame_counts(output_rows)),
        "output_candidates_per_frame_p95": _IMPL._quantile(
            _IMPL._frame_counts(output_rows),
            0.95,
        ),
        "output_candidates_per_frame_max": _IMPL._max(_IMPL._frame_counts(output_rows)),
...
def main(argv: list[str] | None = None) -> int:
    """Run the diversity cap and keep its summary branch semantics aligned."""
