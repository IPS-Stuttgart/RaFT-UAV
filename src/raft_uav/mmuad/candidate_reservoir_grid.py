...
summaries when truth/reference rows are available.  It is intended for the
MMUAD top-3 work item where raw/full-pool candidates must be preserved without
trusting a single ranker score too early.
"""
...
def run_candidate_reservoir_offset_grid(
    ...
    *,
    ...
    Returns the summary table and, when requested, the best reservoir DataFrame.
    """
...
def _sort_summary(summary: pd.DataFrame, *, selection_metric: str) -> pd.DataFrame:
    ...
    if selection_metric in summary.columns:
        ...
        return summary.assign(_sort_metric=values).sort_values(
            ["_sort_metric", "grid_label"],
            na_position="last",
        ).drop(columns=["_sort_metric"]).reset_index(drop=True)
...
def _write_outputs(
    *,
    ...
    summary_json.write_text(
        json.dumps(summary_frame.to_dict(orient="records"), indent=2),
        encoding="utf-8",
    )
    if best_reservoir is not None:
        best_reservoir.to_csv(output_dir / "best_candidate_reservoir.csv", index=False)
    for label, frame_rows in frame_tables.items():
        path = output_dir / f"oracle_frames_{_sanitize_label(label)}.csv"
        frame_rows.to_csv(path, index=False)
    for label, by_sequence in by_sequence_tables.items():
        path = output_dir / f"oracle_by_sequence_{_sanitize_label(label)}.csv"
        by_sequence.to_csv(path, index=False)
