...
def attach_candidate_branch_consensus(
    ...
    *,
    ...
    resolved_branch = _resolve_column(out, branch_column, DEFAULT_BRANCH_COLUMN_ALIASES)
    resolved_origin = _resolve_column(out, origin_column, DEFAULT_ORIGIN_COLUMN_ALIASES)
    out["candidate_branch"] = _branch_values(out, resolved_branch)
    out["_branch_consensus_row_id"] = np.arange(len(out), dtype=int)
    out["branch_consensus_base_score"] = _base_score(out, base_score_column)
    out["branch_consensus_base_score_normalized"] = _frame_minmax_score(
        out,
    ...
    for _, sequence_rows in out.groupby("sequence_id", sort=False):
        ...
        branches = ordered["candidate_branch"].fillna("").astype(str).to_numpy(object)
        origins = _origin_values(ordered, resolved_origin)
        for local_index, global_index in enumerate(ordered_indices):
            lower = int(np.searchsorted(times, times[local_index] - float(time_window_s), side="left"))
            upper = int(np.searchsorted(times, times[local_index] + float(time_window_s), side="right"))
            ...
            if exclude_same_origin_support and origins is not None:
                ...
                if current_origin is not None:
                    different_origin = np.array(
                        [origin is None or origin != current_origin for origin in origins[candidate_indices]],
                        dtype=bool,
                    )
                    candidate_indices = candidate_indices[different_origin]
            if candidate_indices.size == 0:
                continue
            distances = np.linalg.norm(xyz[candidate_indices] - xyz[local_index], axis=1)
            ...
            if supported.any():
                ...
                unique_source_count[global_index] = int(len(set(sources[support_indices])))
                unique_branch_count[global_index] = int(len(set(branches[support_indices])))
    ...
    support_score = 1.0 - np.exp(-neighbor_count.astype(float))
    out["branch_consensus_distance_score"] = distance_score
    out["branch_consensus_support_score"] = support_score
    out["branch_consensus_score"] = 0.7 * distance_score + 0.3 * support_score
    ...
    rank_score = (
        float(base_score_weight) * out["branch_consensus_base_score_normalized"].to_numpy(float)
    ...
    out["branch_consensus_rank_percentile"] = _frame_minmax_score(out, score_output_column)
    if replace_confidence:
        out["confidence"] = rank_score
    return CandidateFrame(
        normalize_candidate_columns(out.drop(columns=["_branch_consensus_row_id"]))
    )
...
def branch_consensus_summary(
    ...
    *,
    ...
    nearest = pd.to_numeric(
        rows.get("branch_consensus_nearest_cross_source_distance_m"),
        errors="coerce",
    )
    finite_nearest = nearest[np.isfinite(nearest.to_numpy(float))]
    pair_advantage = pd.to_numeric(
        rows.get("branch_consensus_pair_advantage_m"),
        errors="coerce",
    )
    finite_pair = pair_advantage[np.isfinite(pair_advantage.to_numpy(float))]
    score = pd.to_numeric(rows.get(score_output_column), errors="coerce")
    finite_score = score[np.isfinite(score.to_numpy(float))]
    return {
        "row_count": int(len(rows)),
        "sequence_count": int(rows["sequence_id"].astype(str).nunique()) if not rows.empty else 0,
        ...
        "cross_source_match_fraction": float(len(finite_nearest) / len(rows)) if len(rows) else 0.0,
        "nearest_cross_source_distance_mean_m": _safe_mean(finite_nearest),
        "nearest_cross_source_distance_p95_m": _safe_quantile(finite_nearest, 0.95),
        "paired_hypothesis_count": int(len(finite_pair)),
        "paired_hypothesis_positive_advantage_count": int((finite_pair > 0.0).sum()),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="attach cross-sensor branch-consensus ranking features to MMUAD candidates",
    )
...
def _resolve_column(
    ...
    for alias in aliases:
        if str(alias).lower() in lower_to_original:
            return lower_to_original[str(alias).lower()]
    return None
...
def _origin_values(rows: pd.DataFrame, origin_column: str | None) -> np.ndarray | None:
...
def _frame_minmax_score(rows: pd.DataFrame, column: str) -> pd.Series:
    values = pd.to_numeric(rows[column], errors="coerce")
    result = pd.Series(np.zeros(len(rows), dtype=float), index=rows.index)
    for _, group in rows.assign(_score=values).groupby(["sequence_id", "time_s"], sort=False):
        group_values = pd.to_numeric(group["_score"], errors="coerce")
...
def _pair_consensus_advantage(
    ...
    *,
    ...
    valid_origin = rows[origin_column].notna() & rows[origin_column].astype(str).str.strip().ne("")
    for _, group in rows.loc[valid_origin].groupby(group_keys, sort=False, dropna=False):
        if len(group) < 2:
            continue
        distances = pd.to_numeric(group[distance_column], errors="coerce")
        for row_index in group.index:
            current = float(distances.loc[row_index]) if np.isfinite(distances.loc[row_index]) else np.nan
            siblings = distances.drop(index=row_index)
            siblings = siblings[np.isfinite(siblings.to_numpy(float))]
            if not np.isfinite(current) or siblings.empty:
                continue
            advantage[int(row_index)] = float(siblings.min() - current)
    return advantage
