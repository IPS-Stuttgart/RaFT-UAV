"""Entropy-adaptive pair-state forward-backward priors for MMUAD candidates.

The acceleration-aware pair-state forward-backward model can recover a coherent
trajectory when the per-frame ranker buries the useful candidate.  Its posterior
can also become overconfident when the candidate graph contains one smooth but
wrong mode.  This module blends the local emission posterior with the global
pair-state posterior using a framewise confidence derived from normalized
posterior entropy.

Ambiguous pair-state posteriors therefore back off toward the local learned
score/uncertainty model, while decisive pair-state posteriors retain the global
trajectory evidence.  The blend is inference-safe: it uses candidate scores,
learned uncertainty, geometry, timestamps, and metadata only.
"""
...
DEFAULT_OUTPUT_SCORE_COLUMN = "candidate_pair_forward_backward_adaptive_score"


@dataclass(frozen=True)
class EntropyAdaptivePairBlendConfig:
    """Configuration for confidence-adaptive local/global posterior blending."""
...
def blend_candidate_posteriors(
    ...
    *,
    config: EntropyAdaptivePairBlendConfig | None = None,
) -> tuple[np.ndarray, dict[str, float]]:
    """Blend two frame-normalized posteriors with entropy-adaptive strength.

    The blend is geometric in probability space.  This retains the ranking
    semantics of both experts and avoids arithmetic averaging assigning large
    mass to a candidate rejected by either expert.
    """

    cfg = config or EntropyAdaptivePairBlendConfig()
    ...
    if len(pair) <= 1:
        ...
        normalized_entropy = float(np.clip(entropy / np.log(len(pair)), 0.0, 1.0))
...
def attach_entropy_adaptive_pair_prior(
    ...
    *,
    ...
    """Attach an entropy-adaptive pair-state posterior to candidate rows."""
    ...
    for _, frame in rows.groupby(["sequence_id", "time_s"], sort=False):
        ...
        rows.loc[indices, "candidate_pair_forward_backward_adaptive_rank"] = _descending_ranks(
            blended
        )
        rows.loc[indices, "candidate_pair_forward_backward_adaptive_pair_entropy"] = (
            diagnostics["pair_entropy"]
        )
        rows.loc[
            indices,
            "candidate_pair_forward_backward_adaptive_normalized_entropy",
        ] = diagnostics["pair_normalized_entropy"]
        rows.loc[indices, "candidate_pair_forward_backward_adaptive_confidence"] = (
            diagnostics["pair_confidence"]
        )
        rows.loc[indices, "candidate_pair_forward_backward_adaptive_pair_weight"] = (
            diagnostics["effective_pair_weight"]
        )
...
def entropy_adaptive_pair_summary(
    ...
    *,
    ...
    """Return compact diagnostics for entropy-adaptive candidate posteriors."""
...
def write_entropy_adaptive_pair_outputs(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "schema": "raft-uav-mmuad-entropy-adaptive-pair-forward-backward-v1",
        ...
        "adaptive_summary": entropy_adaptive_pair_summary(
            candidates,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="mmuad-candidate-pair-forward-backward-adaptive",
        description="blend local and pair-state MMUAD candidate posteriors by entropy confidence",
    )
    ...
    parser.add_argument("--score-normalization", choices=("minmax", "rank", "none"), default="minmax")
    ...
    blend_cfg = EntropyAdaptivePairBlendConfig(
    ...
    print("mmuad_entropy_adaptive_pair_forward_backward=ok")
...
def _initialize_output_columns(rows: pd.DataFrame, output_score_column: str) -> None:
    for column in (
        ...
        "candidate_pair_forward_backward_adaptive_pair_entropy",
        "candidate_pair_forward_backward_adaptive_normalized_entropy",
        "candidate_pair_forward_backward_adaptive_confidence",
        "candidate_pair_forward_backward_adaptive_pair_weight",
    ):
        rows[column] = np.nan
...
def _top_candidate_ids(rows: pd.DataFrame, score_column: str) -> pd.Series:
    ...
    scored = rows.copy()
    scored["_adaptive_score"] = pd.to_numeric(scored[score_column], errors="coerce")
    scored["_adaptive_row_id"] = np.arange(len(scored), dtype=int)
    top = (
        scored.sort_values(
            ["sequence_id", "time_s", "_adaptive_score", "_adaptive_row_id"],
    ...
    value = top.get("track_id", top["_adaptive_row_id"]).astype(str)
...
def _validate_blend_config(config: EntropyAdaptivePairBlendConfig) -> None:
    values = (
    ...
    if not all(np.isfinite(float(value)) for value in values):
        raise ValueError("adaptive blend parameters must be finite")
    ...
    if float(config.confidence_power) <= 0.0:
