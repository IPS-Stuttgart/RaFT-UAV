...
def select_adaptive_mixture_candidates(
    ...
    *,
    ...
    rows[config.branch_column] = rows[config.branch_column].fillna("candidate").astype(str)
    ...
    for _, frame in rows.groupby(["sequence_id", "time_s"], sort=False, dropna=False):
        ...
        adaptive_k = _adaptive_top_k(frame, diagnostics=diagnostics, config=config)
        ...
        stratified["mixture_adaptive_score_entropy"] = diagnostics["score_entropy"]
...
def run_adaptive_candidate_mixture_map(
    ...
    *,
    ...
    selected = select_adaptive_mixture_candidates(candidates, config=adaptive_config)
    effective_mixture_config = replace(mixture_config, top_k=int(adaptive_config.max_top_k))
...
def build_adaptive_selection_summary(
    ...
    *,
    ...
    selected = normalize_candidate_columns(pd.DataFrame(selected_candidates).copy())
    frame_rows = (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="run ambiguity-adaptive stratified MMUAD candidate-mixture smoothing",
    )
    ...
    parser.add_argument("--adaptive-score-temperature", type=float, default=1.0)
...
def _frame_ambiguity(
    ...
    *,
    ...
    score = _candidate_score(frame, config=config)
    normalized = _minmax(score)
    probabilities = _softmax(normalized, temperature=config.score_temperature)
    ...
    margin_ambiguity = 1.0 - float(np.clip(margin / config.margin_scale, 0.0, 1.0))

    sigma = _numeric_column(frame, config.sigma_column, default=np.nan).to_numpy(float)
    finite_sigma = np.isfinite(sigma)
    sigma_ambiguity = float("nan")
    if finite_sigma.any():
        weights = probabilities[finite_sigma]
        weights = weights / weights.sum() if weights.sum() > 0 else np.full_like(weights, 1.0 / len(weights))
    ...
    consensus = _numeric_column(frame, config.consensus_column, default=np.nan).to_numpy(float)
    finite_consensus = np.isfinite(consensus)
    consensus_ambiguity = float("nan")
    if finite_consensus.any():
        weights = probabilities[finite_consensus]
        weights = weights / weights.sum() if weights.sum() > 0 else np.full_like(weights, 1.0 / len(weights))
        weighted_consensus = float(np.sum(weights * consensus[finite_consensus]))
    ...
    available = [(float(weight), float(value)) for weight, value in weighted_terms if weight > 0 and np.isfinite(value)]
...
def _adaptive_top_k(
    ...
    *,
    ...
    span = int(config.max_top_k) - int(config.min_top_k)
    adaptive = int(np.ceil(int(config.min_top_k) + diagnostics["ambiguity"] * span))
    branch_count = frame[config.branch_column].fillna("candidate").astype(str).nunique()
    source_count = frame["source"].fillna("candidate").astype(str).nunique()
    source_branch_count = frame[["source", config.branch_column]].fillna("candidate").drop_duplicates().shape[0]
    quota_floor = max(
        int(config.min_top_k),
    ...
    return int(
        np.clip(
            max(adaptive, quota_floor),
            int(config.min_top_k),
            min(int(config.max_top_k), len(frame)),
        )
    )


def _candidate_score(frame: pd.DataFrame, *, config: AdaptiveMixtureTopKConfig) -> np.ndarray:
    values = _numeric_column(frame, config.score_column, default=np.nan)
    for column in config.fallback_score_columns:
        values = values.fillna(_numeric_column(frame, column, default=np.nan))
...
def _softmax(values: np.ndarray, *, temperature: float) -> np.ndarray:
    ...
    return exp_values / total if total > 0 else np.full(len(logits), 1.0 / len(logits))
