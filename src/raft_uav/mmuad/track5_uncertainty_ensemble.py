    ...
    *,
    ...
    for item in inputs:
        ...
        part["inverse_variance_weight"] = np.where(
            np.isfinite(part["predicted_sigma_m"].to_numpy(float))
            & (part["predicted_sigma_m"].to_numpy(float) > 0.0),
            float(item.weight) / np.square(part["predicted_sigma_m"].to_numpy(float)),
            0.0,
        )
