"""Block diagnostics for MMUAD candidate-mixture assignment failures.

``raft-uav-mmuad-candidate-assignment-diagnostics`` produces one row per
mixture frame.  This module groups those rows into contiguous time blocks so a
server run can quickly distinguish one-off wrong assignments from persistent
segments where the mixture MAP smoother is following the wrong branch or the
best retained candidate is missing.

The tool is diagnostic-only: it consumes local truth-derived assignment rows and
is not part of hidden-test inference.
"""
...
def build_candidate_assignment_block_tables(
    ...
    *,
    ...
    """Return contiguous block and summary tables for assignment diagnostics."""
    ...
    return blocks, build_candidate_assignment_block_summary(blocks)
...
def build_candidate_assignment_block_summary(blocks: pd.DataFrame) -> pd.DataFrame:
    ...
    records.append(_summarize_group(rows, sequence_id="__pooled__", failure_mode="__all__"))
    for mode, group in rows.groupby("assignment_failure_mode", sort=True):
        records.append(_summarize_group(group, sequence_id="__pooled__", failure_mode=str(mode)))
    for sequence_id, group in rows.groupby("sequence_id", sort=True):
        records.append(_summarize_group(group, sequence_id=str(sequence_id), failure_mode="__all__"))
        for mode, mode_group in group.groupby("assignment_failure_mode", sort=True):
            records.append(
                _summarize_group(mode_group, sequence_id=str(sequence_id), failure_mode=str(mode)),
            )
...
def _summarize_block(
    ...
    *,
    ...
    return {
        ...
        "oracle_error_3d_m_p95": _quantile(frame.get("oracle_error_3d_m"), 0.95),
        "oracle_error_3d_m_max": _max(frame.get("oracle_error_3d_m")),
        "state_regret_m_mean": _mean(frame.get("state_regret_m")),
        "state_regret_m_p95": _quantile(frame.get("state_regret_m"), 0.95),
        "state_regret_m_max": _max(frame.get("state_regret_m")),
        "dominant_regret_m_mean": _mean(frame.get("dominant_regret_m")),
        "dominant_regret_m_p95": _quantile(frame.get("dominant_regret_m"), 0.95),
        "dominant_regret_m_max": _max(frame.get("dominant_regret_m")),
        "weighted_regret_m_mean": _mean(frame.get("weighted_regret_m")),
        "oracle_weight_rank_mean": _mean(frame.get("oracle_weight_rank")),
        ...
        "dominant_mixture_branch": _mode(frame.get("dominant_candidate_branch")),
        "dominant_oracle_source": _mode(frame.get("oracle_source")),
        "dominant_mixture_source": _mode(frame.get("dominant_source")),
        "assignment_entropy_mean": _mean(frame.get("assignment_entropy")),
        "oracle_in_topk_by_weight_rate": _mean(_bool_as_float(frame.get("oracle_in_topk_by_weight"))),
        "dominant_matches_oracle_rate": _mean(_bool_as_float(frame.get("dominant_is_oracle"))),
    }


def _summarize_group(group: pd.DataFrame, *, sequence_id: str, failure_mode: str) -> dict[str, Any]:
    frames = pd.to_numeric(group.get("frame_count"), errors="coerce").dropna()
    durations = pd.to_numeric(group.get("duration_s"), errors="coerce").dropna()
    state_max = pd.to_numeric(group.get("state_error_3d_m_max"), errors="coerce").dropna()
    state_regret_max = pd.to_numeric(group.get("state_regret_m_max"), errors="coerce").dropna()
    return {
        "sequence_id": sequence_id,
        ...
        "state_error_3d_m_block_max": _max(state_max),
        "state_error_3d_m_block_p95": _quantile(state_max, 0.95),
        "state_regret_m_block_max": _max(state_regret_max),
        "state_regret_m_block_p95": _quantile(state_regret_max, 0.95),
    }
...
def _bool_as_float(values: pd.Series | None) -> pd.Series:
    if values is None:
        return pd.Series(dtype=float)
    if values.dtype == bool:
        return values.astype(float)
    text = values.fillna(False).astype(str).str.lower()
    return text.isin({"true", "1", "yes", "y"}).astype(float)
