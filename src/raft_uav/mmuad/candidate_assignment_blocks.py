"""Block diagnostics for MMUAD candidate-assignment failures.

``candidate_assignment_diagnostics`` is frame-level.  This module turns those
rows into contiguous failure blocks so a reservoir/mixture run can distinguish
short isolated assignment mistakes from persistent intervals where the oracle
candidate is buried, missing, or the smoothed state stays away from a good
assignment.
"""
...
def build_candidate_assignment_block_tables(
    ...
    *,
    ...
    """Return contiguous assignment-failure block and summary tables."""
...
def build_candidate_assignment_block_summary(blocks: pd.DataFrame) -> pd.DataFrame:
    """Summarize assignment-failure blocks by sequence and mode."""

    rows = pd.DataFrame(blocks).copy()
    if rows.empty:
        return pd.DataFrame()
    records: list[dict[str, Any]] = []
    records.append(_summarize_block_group(rows, sequence_id="__pooled__", mode="__all__"))
    ...
    for sequence_id, group in rows.groupby("sequence_id", sort=True):
        records.append(_summarize_block_group(group, sequence_id=str(sequence_id), mode="__all__"))
        for mode, mode_group in group.groupby("assignment_failure_mode", sort=True):
            records.append(
                _summarize_block_group(mode_group, sequence_id=str(sequence_id), mode=str(mode))
            )
...
def write_candidate_assignment_block_outputs(
    *,
    ...
    """Write assignment block artifacts and return their paths."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="summarize contiguous MMUAD candidate-assignment failure blocks",
    )
    ...
    paths = write_candidate_assignment_block_outputs(
        ...
        blocks=blocks,
        summary=summary,
        provenance={"frame_csv": str(args.frame_csv), "max_gap_s": float(args.max_gap_s)},
    )
...
def _summarize_block(
    ...
    *,
    ...
    return {
        ...
        "oracle_in_topk_by_weight_rate": _mean(_bool_to_float(frame.get("oracle_in_topk_by_weight"))),
        "dominant_matches_oracle_rate": _mean(_bool_to_float(frame.get("dominant_is_oracle"))),
        "dominant_oracle_branch": _mode(frame.get("oracle_candidate_branch")),
        "dominant_oracle_source": _mode(frame.get("oracle_source")),
        "dominant_assignment_branch": _mode(frame.get("dominant_candidate_branch")),
        "dominant_assignment_source": _mode(frame.get("dominant_source")),
    }
...
def _summarize_block_group(group: pd.DataFrame, *, sequence_id: str, mode: str) -> dict[str, Any]:
    ...
    max_errors = pd.to_numeric(group.get("state_error_3d_m_max"), errors="coerce").dropna()
    return {
        "sequence_id": sequence_id,
        "assignment_failure_mode": mode,
        ...
        "state_error_3d_m_block_max": _max(max_errors),
        "state_error_3d_m_block_p95": _quantile(max_errors, 0.95),
        "state_regret_m_mean": _mean(group.get("state_regret_m_mean")),
        "dominant_oracle_branch": _mode(group.get("dominant_oracle_branch")),
        "dominant_assignment_branch": _mode(group.get("dominant_assignment_branch")),
    }
...
def _bool_to_float(values: pd.Series | None) -> pd.Series:
    if values is None:
