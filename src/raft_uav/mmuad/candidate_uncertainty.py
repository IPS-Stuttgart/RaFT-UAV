...
The MMUAD candidate-mixture experiments benefit from a candidate-specific
measurement scale rather than one fixed covariance for every cluster.  This
module learns expected candidate position error from train truth and existing
cluster-ranker features, then writes a clipped ``predicted_sigma_m`` column for
robust mixture-MAP or tracker experiments.

Class-probability, source, and candidate-branch interactions are consumed
implicitly when they are already present as numeric ``image_*`` feature columns.
No validation/test truth is required when applying a saved model.
"""
...
from raft_uav.mmuad.cluster_ranker import BASE_CLUSTER_FEATURE_COLUMNS, build_cluster_feature_table
...
def train_candidate_uncertainty(
    ...
    *,
    ...
    source_values = sorted(rows.get("source", pd.Series("", index=rows.index)).fillna("").astype(str).unique())
...
def predict_candidate_sigma(
    ...
    matrix = _feature_matrix(rows, model.feature_columns, source_values=model.source_values)
...
def candidate_uncertainty_training_summary(
    ...
    truth = pd.to_numeric(features.get("truth_distance_3d_m"), errors="coerce")
    ...
    if len(truth_values) >= 2 and np.std(truth_values) > 0.0 and np.std(predicted_values) > 0.0:
...
def save_candidate_uncertainty_model(model: CandidateUncertaintyModel, path: Path) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="raft-uav-mmuad-candidate-uncertainty",
    ...
    apply.add_argument("--candidates-csv", type=Path, required=True)
    apply.add_argument("--model-json", type=Path, required=True)
    apply.add_argument("--output-csv", type=Path, required=True)
    apply.add_argument("--output-column", default="predicted_sigma_m")
    apply.add_argument("--replace-covariance", action="store_true")
    apply.add_argument("--z-scale", type=float, default=1.0)
...
def _uncertainty_feature_columns(rows: pd.DataFrame, *, source_values: list[str]) -> list[str]:
    columns: list[str] = []
    ...
    *,
    ...
    for column in feature_columns:
        if column.startswith(_SOURCE_PREFIX):
            ...
            values.append(
                pd.to_numeric(rows.get(column, pd.Series(np.nan, index=rows.index)), errors="coerce")
                .to_numpy(float)
            )
    if not values:
...
def _make_sklearn_regressor(*, model_type: str, random_state: int, n_estimators: int) -> Any:
    try:
