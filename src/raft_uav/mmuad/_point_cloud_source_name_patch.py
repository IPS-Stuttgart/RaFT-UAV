...
def _logical_data_stem(path: Path, *, data_file_suffix) -> str:
    """Return a filename without transparent compression or data suffixes."""
...
def install() -> None:
    ...
    # The MMUAD package imports sequence helpers while the package itself is being
    # initialized. Update those already-bound aliases as well as the public module.
    for module_name in ("raft_uav.mmuad.sequence", "raft_uav.mmuad.cli"):
        consumer = sys.modules.get(module_name)
        if consumer is None:
            continue
        if getattr(consumer, "load_point_cloud_file_as_points", None) is original_points:
            consumer.load_point_cloud_file_as_points = load_point_cloud_file_as_points
        if (
            getattr(consumer, "load_point_cloud_file_as_candidates", None)
            is original_candidates
        ):
            consumer.load_point_cloud_file_as_candidates = (
                load_point_cloud_file_as_candidates
            )
