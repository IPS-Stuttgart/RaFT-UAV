...
def position_errors_m(
    ...
    """Compute nearest-neighbor position errors against truth."""
    ...
    deltas_xyz = estimate_positions[:, :dimensions] - truth_positions[truth_indices, :dimensions]
    errors = np.linalg.norm(deltas_xyz, axis=1)
    finite = np.isfinite(errors) & np.isfinite(deltas_t)
    errors = errors[finite]
    deltas_t = deltas_t[finite]
