...
def _stable_nanmean_nonnegative(values: np.ndarray) -> float:
    """Return a NaN-skipping mean without overflowing finite nonnegative inputs."""
    ...
    with np.errstate(under="ignore"):
        scaled = valid / max_value
    return float(max_value * np.mean(scaled))
...
def frame_context_features(candidates: pd.DataFrame) -> pd.DataFrame:
    ...
    if candidate_count > 1:
        ...
        out["mean_neighbor_distance_m"] = np.array(
...
def _tracklet_features(segment: pd.DataFrame, track_id: object, segment_index: int) -> dict[str, object]:
    ...
    return {
        ...
        "mean_range_m": _stable_nanmean_nonnegative(ranges),
