...
def _baseline_metrics_with_runtime_config(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    metrics["radar_update"] = _radar_update_description(runtime_config)
    return metrics
...
def _radar_update_description(runtime_config: dict[str, Any]) -> dict[str, Any]:
    radar_update = runtime_config.get("radar_update", {})
    dimension = str(radar_update.get("dimension", "position"))
    return {
        "dimension": dimension,
        "uses_velocity": dimension == "position-velocity",
        "velocity_std_mps": float(radar_update.get("velocity_std_mps", 12.0)),
    }
