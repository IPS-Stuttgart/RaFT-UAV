...
def run_async_cv_baseline_with_tracklet_viterbi_result(
    *,
    ...
    ``accepted_radar`` contains only radar rows accepted by Kalman replay, while
    ``viterbi_selected_radar`` contains every non-miss Viterbi choice annotated
    with replay acceptance, NIS, residual norm, and gating diagnostics.
    """
    ...
    if not events:
        ...
        return TrackletViterbiResult([], empty, _empty_replayed_rows(empty))
    ...
    return TrackletViterbiResult(records, accepted_frame, replayed_frame)
