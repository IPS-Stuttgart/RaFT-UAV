...
def apply_radar_candidate_index_patch(module: ModuleType) -> None:
    ...
    Several association modes select the minimum-scoring row with ``idxmin``
    followed by label-based ``.loc``.  Duplicate input labels make that lookup
    return a DataFrame instead of the required Series.  Normalizing only scored
    frames with duplicate labels keeps ordinary caller-visible indices intact.
    """
