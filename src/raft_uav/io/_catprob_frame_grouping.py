...
def _catprob_frame_group_keys(radar: pd.DataFrame) -> list[tuple[Any, ...]]:
    ...
    usable frame or time metadata remain distinct so unrelated detections are not
    collapsed into one synthetic frame.
    """
    ...
    keys: list[tuple[Any, ...]] = []
    for position, (frame_index, time_key) in enumerate(rows):
        if frame_index is not None and time_key is not None:
            ...
            key = ("__missing_frame__", position)
        keys.append(key)
    return keys
