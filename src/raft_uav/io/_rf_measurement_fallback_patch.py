...
def _rf_measurements_to_enu(
    ...
    if "east_m" in frame.columns:
        raw_std = (
            frame["std_m"].to_numpy(dtype=object)
            if "std_m" in frame.columns
            else np.full(len(frame), None, dtype=object)
        )
        sanitized_std = np.array(
            [std if (std := _positive_std(value)) is not None else default_std for value in raw_std],
            dtype=float,
        )
        frame = frame.copy()
        frame["std_m"] = sanitized_std
