    ...
    for method in resample_methods:
        ...
        for gap_s in max_interpolation_gaps_s:
            for class_policy in classification_policies:
                _validate_choice(class_policy, CLASSIFICATION_POLICIES, "classification_policy")
                _validate_choice(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--truth", type=Path, help="optional public validation reference for local scoring")
    ...
    if args.require_at_least_one_leaderboard_ready and not bool(summary["leaderboard_ready"].any()):
...
def _summary_record(
    *,
    ...
    return {
        ...
        "validation_blocking_reasons": ";".join(validation.get("leaderboard_blocking_reasons", [])),
        ...
        "pose_mean_error_3d_m": _optional_float(public_pooled.get("mean_position_error_3d_m")),
        "pose_p95_error_3d_m": _optional_float(public_pooled.get("p95_position_error_3d_m")),
        "pose_max_error_3d_m": _optional_float(public_pooled.get("max_position_error_3d_m")),
        "uav_type_accuracy": _optional_float(
            public_pooled.get("uav_type_accuracy", public_pooled.get("classification_accuracy")),
        ),
