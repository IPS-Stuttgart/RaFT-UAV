...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/smoothing_ablation"))
    parser.add_argument(
        "--summary-output",
        type=Path,
        default=Path("outputs/smoothing_ablation_opt1_opt3.csv"),
    )
    ...
    configs = [
        ...
        _Config("soft_prediction_nis_rts", association="prediction-nis", robust=True, smoother="rts"),
    ]
...
class _Config:
    def __init__(self, name: str, *, association: str, robust: bool, smoother: str) -> None:
        self.name = name
...
def _run_one(
    *,
    ...
    if config.smoother == "fixed-lag":
        command.extend(["--smoother-lag-s", str(args.fixed_lag_s)])
    if config.robust:
        command.extend(
            [
                "--robust-update",
