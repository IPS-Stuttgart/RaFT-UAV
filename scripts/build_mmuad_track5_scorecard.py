#!/usr/bin/env python
"""Build a local UG2+/MMUAD Track 5 validation/evaluation scorecard."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="build_mmuad_track5_scorecard.py",
        description="combine Track 5 upload validation and local evaluation artifacts",
    )
    ...
    parser.add_argument("--template", type=Path, help="official template/results file for timestamp coverage")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="exit nonzero unless validation and public Track 5 evaluation are leaderboard-ready",
    )
    ...
    scorecard = build_track5_scorecard(
        ...
        nearest_time_delta_s=args.nearest_time_delta_s,
    )
    ...
    pooled = summary.get("public_track5", {}).get("pooled", {})
    ...
    if args.require_leaderboard_ready and not summary["leaderboard_ready"]:
