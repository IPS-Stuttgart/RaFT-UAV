#!/usr/bin/env python
"""Run stratified MMUAD candidate-mixture smoothing and write official ZIPs.

The core ``raft-uav-mmuad-stratified-candidate-mixture-map`` entrypoint writes
candidate-selection and mixture diagnostics.  This helper is intentionally thin:
it reuses the same inference-safe stratified selector and mixture-MAP solver,
then writes official Track 5 CSV/ZIP artifacts in the same command.  That makes
branch-preserving mixture experiments easier to score or submit without a second
...
from raft_uav.mmuad.candidate_mixture_map import (  # noqa: E402
    INITIALIZATION_CHOICES,
    LOSS_CHOICES,
    SCORE_NORMALIZATION_CHOICES,
    CandidateMixtureMapConfig,
)
from raft_uav.mmuad.candidate_mixture_map_stratified import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--score-normalization", choices=SCORE_NORMALIZATION_CHOICES, default="minmax")
    ...
    parser.add_argument("--initialization", choices=INITIALIZATION_CHOICES, default="uncertainty-top1")
