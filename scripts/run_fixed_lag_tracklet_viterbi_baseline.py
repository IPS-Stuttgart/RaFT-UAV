"""Run fixed-lag Tracklet-Viterbi radar association on one flight."""
...
from raft_uav.baselines.tracklet_viterbi_fixed_lag import (  # noqa: E402
...
from run_tracklet_viterbi_baseline import (  # noqa: E402
    _inside_truth_window,
    _metrics,
    _records_to_frame,
    _selected_radar_diagnostics,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/tracklet_viterbi_fixed_lag"))
    ...
    parser.add_argument("--tracklet-viterbi-lag-s", type=float, default=20.0)
    ...
    robust_updates = (
        None if args.robust_update == "none" else {"rf": args.robust_update, "radar": args.robust_update}
    )
    inflation_alphas = None if robust_updates is None else {
        "rf": args.rf_inflation_alpha,
        "radar": args.radar_inflation_alpha,
    }

    records, selected_radar, viterbi_selected_radar = (
        run_async_cv_baseline_with_fixed_lag_tracklet_viterbi_association_and_replay(
            rf_measurements=rf_measurements,
            radar=radar,
            lag_s=args.tracklet_viterbi_lag_s,
    ...
    metrics["tracklet_viterbi"]["association_horizon"] = "fixed-lag"
    metrics["tracklet_viterbi"]["association_lag_s"] = args.tracklet_viterbi_lag_s
    ...
    print("radar_association=tracklet-viterbi-fixed-lag")
    print(f"tracklet_viterbi_lag_s={args.tracklet_viterbi_lag_s}")
