...
def test_class_probability_confidence_entropy_has_expected_endpoints() -> None:
...
def test_confidence_adaptive_weights_back_off_for_uniform_posterior() -> None:
    labels = ["left", "right"]
...
def test_confidence_floor_retains_bounded_class_conditioning() -> None:
    labels = ["left", "right"]
    base_weights = {"left": 1.0, "right": 1.0}
    class_weights = resolve_anchor_class_reliability(
        labels,
    ...
    # The class-average reliability is [3, 7], blended 20% with [1, 1].
...
def test_adaptive_selection_uses_class_only_when_confident() -> None:
    ...
    confident_left = confident.loc[
        confident["track_id"].astype(str).str.contains("-left-")
    ]
    confident_right = confident.loc[
        confident["track_id"].astype(str).str.contains("-right-")
    ]
    ...
    assert set(confident[CONFIDENCE_SCORE_COLUMN]) == {1.0}
    assert set(confident[EFFECTIVE_STRENGTH_COLUMN]) == {1.0}
    assert set(uniform[CONFIDENCE_SCORE_COLUMN]) == pytest.approx({0.0})
    assert set(uniform[EFFECTIVE_STRENGTH_COLUMN]) == pytest.approx({0.0})
    assert set(uniform["mixture_class_conditioned_anchor_weight_left"]) == {1.0}
    assert set(uniform["mixture_class_conditioned_anchor_weight_right"]) == {1.0}
