    ...
    rows = pd.DataFrame(
        {
            ...
            "track_id": [f"candidate-{index}" for index in range(len(flags))],
            "x_m": np.arange(len(flags), dtype=float),
            "y_m": np.zeros(len(flags), dtype=float),
            "z_m": np.ones(len(flags), dtype=float),
