...
def test_exact_frame_gap_join_preserves_valid_one_to_one_rows() -> None:
    estimates = pd.DataFrame(
        {
            "sequence_id": ["seqA"],
            "time_s": [1.0000001],
    ...
    assert len(gap) == 1
    assert gap.loc[0, "mixture_error_3d_m"] == pytest.approx(3.0)
    assert gap.loc[0, "oracle_all_3d_m"] == pytest.approx(1.0)
