...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"bias": np.ma.masked}, "bias must be finite"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"model_type": "ridge", "ridge_alpha": np.ma.masked},
            "ridge_alpha must be finite",
        ),
        (
            {"model_type": "random-forest", "random_state": np.ma.masked},
            "random_state must be finite",
        ),
        (
...
@pytest.mark.parametrize("z_scale", [0.0, np.nan, np.inf, np.ma.masked])
def test_covariance_replacement_rejects_invalid_z_scale(z_scale: object) -> None:
