...
from raft_uav.io.aerpaw import (
    ...
    sensor_clock_offsets_from_environment,
)
...
def test_rf_clock_offset_can_be_overridden_independently(tmp_path):
    truth_path = tmp_path / "vehicleOut.txt"
    truth_path.write_text(
    ...
    rf_path.write_text(
        "\n".join(
            [
                "Meas ID,Algorithm,Center Frequency,Sample Rate,Num Samples,Latitude,Longitude,Elevation,RHO,CEP,Total Sensors,Valid Sensors,Overloaded Sensors,Sensor Names,Time,Comment",
                "1,tdoa,1,1,1,35.72749,-78.69621,0,0,75,4,4,0,s,2025-10-07 15:42:30.000,",
            ]
        ),
        encoding="utf-8",
    )
    ...
    truth, projector, origin_time = normalize_truth(read_truth(truth_path))
    rf = normalize_rf(read_rf_csv(rf_path), projector, origin_time, clock_offset_s=0.0)

    np.testing.assert_allclose(rf["time_s"].to_numpy(), np.array([10.0]), atol=1e-6)


def test_radar_clock_offset_can_be_overridden_independently(tmp_path):
    radar_path = tmp_path / "radar.json"
    radar_path.write_text(
        json.dumps(
            {
                "params": {"globalTime": 1759844550.0},
                "trackData": [
                    {
                        "id": 1,
                        "lla": [35.72749, -78.69621, 30.0],
                        "globalTime": 1759844550.0,
                        "catProb": [0.6, 0.1],
                    },
                ],
            }
    ...
    truth_path = tmp_path / "vehicleOut.txt"
    truth_path.write_text(
        '1,-78.696216,35.7274895,2.717,"(0,0,0)","(0,0,0)",49.6,2025-10-07 13:42:20.000000,4,27',
        encoding="utf-8",
    )
    ...
    np.testing.assert_allclose(radar["time_s"].to_numpy(), np.array([10.0]), atol=1e-6)


def test_environment_clock_offsets_are_separate(monkeypatch):
    monkeypatch.setenv(RF_CLOCK_OFFSET_ENV, "1.25")
    monkeypatch.setenv(RADAR_CLOCK_OFFSET_ENV, "-2.5")

    offsets = sensor_clock_offsets_from_environment()

    assert offsets == {"rf_clock_offset_s": 1.25, "radar_clock_offset_s": -2.5}


def test_default_clock_offsets_preserve_legacy_values(monkeypatch):
    monkeypatch.delenv(RF_CLOCK_OFFSET_ENV, raising=False)
    monkeypatch.delenv(RADAR_CLOCK_OFFSET_ENV, raising=False)

    offsets = sensor_clock_offsets_from_environment()

    assert offsets == {
        "rf_clock_offset_s": DEFAULT_RF_CLOCK_OFFSET_S,
        "radar_clock_offset_s": DEFAULT_RADAR_CLOCK_OFFSET_S,
    }
