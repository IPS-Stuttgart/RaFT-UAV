...
def test_summarize_errors_checks_unmasked_cells_in_partially_masked_array() -> None:
    data = np.empty(2, dtype=object)
    data[0] = _boxed(np.float64(99.0))
    data[1] = _boxed(np.bool_(True))
    errors = np.ma.array(data, mask=[True, False])

    with pytest.raises(ValueError, match="errors_m must not contain Boolean values"):
        summarize_errors(errors)


def test_summarize_errors_ignores_masked_nested_boolean() -> None:
    data = np.empty(1, dtype=object)
    data[0] = _boxed(np.bool_(True))
    errors = np.ma.array(data, mask=[True])
...
def test_summarize_errors_rejects_cyclic_object_containers() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="metric inputs must not contain cyclic object containers",
    ):
        summarize_errors(errors)
