...
def test_rts_ensemble_rejects_normalized_template_alias_collisions(
    extra_column: str,
    extra_value: object,
) -> None:
...
@pytest.mark.parametrize("duplicate_column", ["Sequence", "Timestamp"])
def test_rts_ensemble_rejects_exact_duplicate_template_columns(
    duplicate_column: str,
) -> None:
    if duplicate_column == "Sequence":
        template = pd.DataFrame(
            [["seq0001", "seq9999", 0.0]],
        ...
        template = pd.DataFrame(
            [["seq0001", 0.0, 10.0]],
            columns=["Sequence", "Timestamp", "Timestamp"],
        )
