...
def test_sequence_and_flight_ids_form_joint_diagnostic_scope() -> None:
    estimates = _estimate_rows(["shared", "shared"]).assign(
...
def test_pooled_one_sided_flight_metadata_fails_closed() -> None:
    estimates = _estimate_rows(["shared", "shared"]).assign(
        flight_id=["flight-a", "flight-b"]
    )
    truth = pd.DataFrame(
...
def test_track_switches_are_scoped_to_joint_physical_flight() -> None:
    selected_radar = pd.DataFrame(
        {
