...
def test_jerk_limit_rejects_malformed_grid_rows(
    ...
    submission.loc[12, column] = value

    with pytest.raises(
        ValueError,
        match=r"non-finite time or position values at row indices: 12",
    ):
        repair_track5_jerk_kinks(submission)


def test_jerk_limit_preserves_every_valid_grid_row() -> None:
    submission = _submission_rows()

    repaired, diagnostics = repair_track5_jerk_kinks(submission)

    assert len(repaired) == len(submission)
    assert len(diagnostics) == len(submission)
    assert repaired["time_s"].tolist() == submission["time_s"].tolist()
