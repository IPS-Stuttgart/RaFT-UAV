...
def test_normalize_truth_skips_out_of_range_longitude_rows():
    truth = pd.DataFrame(
        {
            "timestamp_raw": [
                "1970-01-01 00:00:01",
    ...
    assert origin_time == pd.Timestamp("1970-01-01 00:00:02")
    assert normalized["time_s"].tolist() == [0.0]
    assert normalized["longitude"].tolist() == [-78.0]


def test_normalize_rf_skips_out_of_range_longitude_rows():
    rf = pd.DataFrame(
        {
            "Time": [
                "1970-01-01 00:00:01",
...
def test_normalize_radar_skips_out_of_range_longitude_rows():
    radar = pd.DataFrame(
        {
            "global_time_raw_s": [1.0, 2.0],
            "latitude": [35.0, 35.0],
