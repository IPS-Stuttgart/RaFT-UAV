...
def test_acceleration_limit_rejects_invalid_classification_values(
    value: object,
) -> None:
    rows = _submission()
...
def test_acceleration_limit_canonicalizes_integer_equivalent_classification_values() -> None:
    rows = _submission()
    rows["Classification"] = ["0", "1.0", 2.0]

    repaired, _ = repair_track5_acceleration_kinks(rows)

    assert repaired["Classification"].tolist() == [0, 1, 2]
    assert repaired["Classification"].dtype.kind in {"i", "u"}
