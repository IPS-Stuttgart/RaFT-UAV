...
@pytest.mark.parametrize("track_id", ["", "   ", "nan", "NaN", "none", "NONE", "<NA>"])
def test_missing_like_string_track_ids_do_not_share_temporal_features(track_id: str) -> None:
    radar = pd.DataFrame(
        {
            "time_s": [0.0, 1.0],
            "frame_index": [10, 11],
    ...
    assert featured["track_id"].tolist() == [track_id, track_id]
