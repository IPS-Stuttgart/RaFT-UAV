...
def _estimate_rows(**overrides: object) -> pd.DataFrame:
...
def _truth_rows(**overrides: object) -> pd.DataFrame:
    rows: dict[str, object] = {
        "sequence_id": ["shared", "shared"],
        "time_s": [0.0, 1.0],
        "x_m": [0.0, 100.0],
...
def test_mot_metrics_never_cross_match_same_time_flights() -> None:
...
def test_mot_metrics_scope_reused_truth_identity_by_flight() -> None:
...
def test_mot_metrics_allow_one_sided_flight_when_sequence_disambiguates() -> None:
...
def test_mot_metrics_reject_ambiguous_one_sided_flight_metadata() -> None:
    estimates = _estimate_rows(flight_id=["flight_a", "flight_b"])
    truth = _truth_rows()
...
def test_mot_tracker_restarts_state_for_each_physical_flight() -> None:
