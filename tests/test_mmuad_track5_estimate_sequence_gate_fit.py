...
from raft_uav.mmuad.track5_estimate_sequence_gate_fit import fit_track5_estimate_sequence_gate
from raft_uav.mmuad.track5_estimate_sequence_gate_fit import main as sequence_gate_fit_main
from raft_uav.mmuad.track5_estimate_sequence_gate_fit import write_track5_estimate_sequence_gate_fit_outputs
...
def test_estimate_sequence_gate_fit_predicts_apply_weights() -> None:
    ...
    assert result.apply_weights is not None
    assert result.apply_weights["sequence_id"].tolist() == ["seqC"]
    assert result.apply_weights["sequence_gate_weight"].between(0.0, 1.0).all()


def test_estimate_sequence_gate_fit_writes_artifacts_and_apply_submission(tmp_path: Path) -> None:
    ...
    paths = write_track5_estimate_sequence_gate_fit_outputs(
        ...
        apply_base_estimates_path=base,
        apply_alternate_estimates_path=alternate,
        write_apply_submission=True,
        template=_template(),
