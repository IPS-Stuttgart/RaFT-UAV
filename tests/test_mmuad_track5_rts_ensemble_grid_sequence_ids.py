...
def test_rts_ensemble_grid_preserves_zero_padded_sequence_ids(tmp_path: Path) -> None:
    ...
    assert np.isfinite(best["best"]["pose_mse_m2"])
    assert grid.loc[0, "diagnostic_valid_input_count_mean"] == pytest.approx(1.0)
