...
def test_rts_grid_preserves_zero_padded_sequence_ids(tmp_path: Path) -> None:
    ...
    grid, best = run_track5_rts_ensemble_grid_search(
        ...
        process_accel_grid=(0.0,),
        spread_variance_scale_grid=(0.0,),
        score_time_tolerance_s=1.0e-9,
    )

    row = grid.iloc[0]
    assert int(row["matched_row_count"]) == 3
    assert row["diagnostic_valid_input_count_mean"] == pytest.approx(1.0)
    assert np.isfinite(float(row["pose_mse_m2"]))
    assert int(best["best"]["matched_row_count"]) == 3
