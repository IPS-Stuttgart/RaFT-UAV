...
def test_load_sequence_class_map_accepts_numpy_scalar_uav_type(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    def fake_read_csv(path: Path | str) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {
                "sequence_id": pd.Series(["seq0001"], dtype=object),
    ...
    assert submission.load_sequence_class_map(class_map_path) == {"seq0001": "2"}
