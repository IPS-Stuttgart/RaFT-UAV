...
def test_tracklet_features_keep_large_aggregate_means_finite() -> None:
    magnitude = 1.0e308
    radar = pd.DataFrame(
        {
            "track_id": [7, 7, 7],
...
def test_frame_context_keeps_large_neighbor_means_finite() -> None:
    magnitude = 1.0e308
    candidates = pd.DataFrame(
        {
    ...
    diagonal_distance = np.hypot(magnitude, magnitude)
    expected_mean = magnitude * (0.5 + np.sqrt(2.0) / 2.0)
    np.testing.assert_allclose(
        featured["nearest_neighbor_distance_m"].to_numpy(dtype=float),
    ...
    np.testing.assert_allclose(
        featured["mean_neighbor_distance_m"].to_numpy(dtype=float),
        [magnitude, expected_mean, expected_mean],
    )
    assert np.isfinite(diagonal_distance)
