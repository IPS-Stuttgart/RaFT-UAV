...
def test_forward_backward_rejects_nonfinite_config_controls(
    field: str,
    value: float,
) -> None:
...
def test_forward_backward_keeps_finite_numpy_scalar_controls() -> None:
    config = replace(
        CandidateForwardBackwardConfig(),
        score_weight=np.float64(1.0),
        transition_distance_std_m=np.float32(2.0),
    )
