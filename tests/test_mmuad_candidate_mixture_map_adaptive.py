...
def test_adaptive_selector_expands_k_for_ambiguous_frames() -> None:
    ...
    chosen_k = (
        selected.groupby("time_s")["mixture_adaptive_top_k"]
        .first()
        .to_dict()
    )
...
def test_adaptive_ambiguity_can_ignore_deep_low_score_clutter() -> None:
    ...
    assert counts[0.0] == 2
    assert counts[1.0] == 5
    ambiguity_count = selected.groupby("time_s")[
        "mixture_adaptive_ambiguity_input_count"
    ].first()
    assert ambiguity_count.to_dict() == {0.0: 2, 1.0: 2}
