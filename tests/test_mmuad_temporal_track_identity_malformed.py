...
@pytest.mark.parametrize(
    "value",
    [
        pytest.param(1.0 + 0.0j, id="python-complex"),
        pytest.param(np.complex128(1.0 + 0.0j), id="numpy-complex"),
        pytest.param(np.ma.masked, id="masked"),
        pytest.param(np.asarray([7]), id="one-dimensional-array"),
        pytest.param(np.asarray([[7]]), id="two-dimensional-array"),
    ],
)
def test_canonical_track_id_rejects_malformed_scalar_values(value: object) -> None:
    assert canonical_track_id(value) is None


def test_canonical_track_id_accepts_zero_dimensional_numeric_arrays() -> None:
    assert canonical_track_id(np.asarray(491)) == "491"
    assert canonical_track_id(np.asarray(491.0)) == "491"


def test_temporal_prior_does_not_reward_repeated_complex_track_ids() -> None:
    previous = {
        "time_s": 0.0,
        "positions": np.asarray([[0.0, 0.0, 0.0]]),
        "sources": np.asarray(["lidar_360"], dtype=object),
        "branches": np.asarray(["raw"], dtype=object),
        "track_ids": np.asarray([1.0 + 0.0j], dtype=object),
    }
    current = {
        "time_s": 1.0,
        ...
        "track_ids": np.asarray([1.0 + 0.0j, "other"], dtype=object),
    }
